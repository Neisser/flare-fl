...
from flare.federation.aggregation_strategies import AggregationStrategy
...
class Orchestrator(FlareNode):
    ...
    def __init__(self, config: FlareConfig):
        ...
        self.consensus_mechanism: ConsensusMechanism = config.get_required(
            "consensus_mechanism"
        )
        self.aggregation_strategy: AggregationStrategy = config.get_required(
            "aggregation_strategy"
        )

        self.registered_clients: List[Client] = []
        self.current_round_number: int = 0
        self.global_model_weights: ModelWeights = (
            self.model_adapter.get_weights()
    ...
    def select_clients(self) -> List[Client]:
        ...
        print(
            f"Orchestrator: Selected {len(selected_clients)} clients for round {self.current_round_number + 1}."
        )
        return selected_clients

    def _distribute_global_model(
        self, round_context: RoundContext, selected_clients: List[Client]
    ) -> Optional[StorageIdentifier]:
        """
        Serializes and stores the global model, then sends its reference to clients.
        """
        print(
            f"Orchestrator: Distributing global model for round {round_context.round_number}..."
        )
        ...
        if stored_id:
            print(
                f"Orchestrator: Global model stored at '{stored_id}'. Notifying clients."
            )
    ...
    def _collect_updates(
        ...
        """
        print(
            f"Orchestrator: Collecting updates from {len(selected_clients)} clients..."
        )
        ...
        for client in selected_clients:
            ...
            if storage_ref:
                ...
                if compressed_bytes:
                    ...
                    client_weights = self.model_adapter.deserialize_weights(
                        decompressed_bytes
                    )

                    # Validate contribution using consensus mechanism
                    if self.consensus_mechanism.validate_contribution(
                        client_weights, client_id=client.node_id
                    ):
                        ...
                        print(
                            f"Orchestrator: Collected valid update from client {client.node_id}."
                        )
                    else:
                        print(
                            f"Orchestrator: Client {client.node_id} update failed consensus validation. Skipping."
                        )
                else:
                    print(
                        f"Orchestrator: Failed to retrieve compressed update for client {client.node_id}."
                    )
            else:
    ...
    def _aggregate_updates(
        self, collected_updates: List[Dict[str, Any]]
        ...
        print(
            f"Orchestrator: Aggregating {len(updates_only)} updates using {type(self.aggregation_strategy).__name__}."
        )
    ...
    def _log_global_model_on_blockchain(
        self, model_weights: ModelWeights, round_number: int
    ):
        ...
        print(
            f"Orchestrator: Global model for round {round_number} logged on blockchain with hash: {model_hash}."
        )
    ...
    def evaluate_global_model(self, eval_data: EvalData) -> Metrics:
        ...
        )  # Ensure adapter has current global weights
    ...
    def execute_round(self):
        ...
        round_context = RoundContext(
            self.current_round_number, self.global_model_weights
        )
        ...
        global_model_storage_ref = self._distribute_global_model(
            round_context, selected_clients
        )
        ...
        agreed_global_weights = self.consensus_mechanism.reach_agreement(
            proposals=[
                new_global_weights
            ],  # Pass the aggregated weights as the proposal
            round_context=round_context,
        )
        ...
        if agreed_global_weights is not None:
            ...
            print(
                f"Orchestrator: Global model updated for round {self.current_round_number}."
            )
            self._log_global_model_on_blockchain(
                self.global_model_weights, self.current_round_number
            )
        else:
            print(
                f"Orchestrator: Consensus failed for round {self.current_round_number}. Global model not updated."
            )
