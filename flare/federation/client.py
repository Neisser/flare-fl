...
class Client(FlareNode):
    ...
    def __init__(
        ...
        self.data_size = len(local_data) if hasattr(local_data, '__len__') else 1  # For FedAvg weighting
    ...
    def receive_global_model(self, model_ref: StorageIdentifier) -> bool:
        ...
        if model_bytes is None:
            print(f"Client {self.node_id}: Failed to retrieve global model from '{model_ref}'.")
            return False
        ...
        try:
            ...
            print(f"Client {self.node_id}: Global model received and loaded successfully.")
            return True
        except Exception as e:
            print(f"Client {self.node_id}: Error deserializing global model: {e}")
            return False

    def train_local(self, round_context: RoundContext, epochs: int, learning_rate: float) -> Optional[ModelWeights]:
        """
        ...
        print(f"Client {self.node_id}: Starting local training for round {round_context.round_number}...")
        try:
            ...
            print(f"Client {self.node_id}: Local training complete. History: {train_history}")
    ...
    def send_update(self, round_context: RoundContext, updated_weights: ModelWeights) -> Optional[StorageIdentifier]:
        """
        ...
        try:
            ...
            print(f"Client {self.node_id}: Update compressed from {len(weights_bytes)} to {len(compressed_update_bytes)} bytes.")

            # Store the compressed update
            update_id = f"client_{self.node_id}_round_{round_context.round_number}_update"
            ...
            if stored_id:
                ...
                print(f"Client {self.node_id}: Transaction logged on blockchain for round {round_context.round_number}.")
                return stored_id
            else:
